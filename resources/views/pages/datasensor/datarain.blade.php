        ...
        <div class="row g-4">
            ...
            <div class="col-sm-12 col-xl-0">
                <div class="bg-light rounded h-100 p-4">
                    <h5 class="mb-4">Data Sensor Hujan</h5>

                    <table id="table-rain" class="table table-info table-striped table-hover" style="background-color: white;">
                        <thead>
                            <tr>
                                <th>No</th>
                                <th>Value</th>
                        ...
                        <tbody>
                            @foreach ($rains as $rain)
                                <tr>
                                    ...
                                    <td>{{ $rain->value }}</td>
                                    <td>{{ $rain->weather }}</td>
                                    <td>{{ $rain->created_at->timezone('Asia/Jakarta')->format('d M Y, H:i:s') }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            const fileTitle = 'Data_Sensor_Hujan_{{ now()->format('Ymd') }}';

            $('#table-rain').DataTable({
